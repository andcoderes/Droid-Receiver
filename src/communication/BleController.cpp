...
void BleController::loop() {
    if (msgReady_) {
        msgReady_ = false;
        // Snapshot under the lock so onWrite() can't clobber msgBuf_ while
        // callback_ (e.g. JSON parsing) is still reading it.
        char local[MAX_MSG_LEN];
        int len;
        portENTER_CRITICAL(&msgMux_);
        len = msgLen_;
        memcpy(local, msgBuf_, len + 1);  // include null terminator
